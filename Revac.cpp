...
public:
	...
	void evaluate(double(*objective)(const ParaVector&))
	{
		m_fitness = objective(*this);
	}
...
void printRow(const ParaVector& vec)
{
	int i = 0;
	for (auto& el : vec.data())
	{
		std::cout << unscale0_1(el,vec.bounds()[i].min, vec.bounds()[i].max) << "\t";
		i++;
	}
	std::cout << "\n";
}
...
ParaVector REVAC(int k, const Bound*bounds,double (*objective)(const ParaVector&), int p, int h, int s)
{
	// initialization
...
int main()
{
	...
	ParaVector solution = REVAC(k, bounds.data(), rosenbrock, p, h, s);
